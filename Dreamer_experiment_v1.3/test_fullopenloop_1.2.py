...
def main(args):
    ...
    for episode in range(load_weight_episode-1, config.all_episodes):
        ...
        obs, end_sign = env.preparation_for_next_episode(episode+1) #o1を取得
        done = False
        total_reward = 0
        ...
        while not done:
            ...
            if step <= 5:                
                action = policy(obs, 0, training=False)#0はepsilonの計算に用いているが，testでは関係ない
                ...
                with torch.no_grad():
                    ...
                    fully_openl_image[step-1] = predict_image.astype(np.uint8)
